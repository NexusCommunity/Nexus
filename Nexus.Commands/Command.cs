...
namespace Nexus.Commands
{
    ...
    public class Command
    {
        ...
        public Command(CommandAttribute parent, MethodInfo method, ParameterInfo[] parameters, object invoker)
        {
            ...
            Usage = method.GetCustomAttribute<CommandUsageAttribute>()?.Usage ?? string.Join(" ", parameters.Where(x => x.ParameterType != typeof(CommandContext)).Select(x => x.Name));
            ...
            ConvertArguments = method.GetCustomAttribute<ConvertArgumentsAttribute>() != null;
            ...
            Log.Debug("Nexus", $"Command Handler created: {method.DeclaringType.FullName}.{method.Name}");
        }
